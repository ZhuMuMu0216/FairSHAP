...
try:
    ...
    def fantasy_timer(func):
        def wrapper(*args, **kwargs):
            ...
            return result, 0.0  # Return (result, elapsed_time) to match expected format
        return wrapper

    def check_zero(value):
        return 1e-20 if value == 0 else value

    def judge_transform_need(values):
        try:
            unique_vals = sorted(set(values))
        except TypeError:
            unique_vals = list(dict.fromkeys(values))
        return unique_vals, False
